/**
 * useApexDashboard Hook
 * Fetches dashboard summary data from Signal Horizon API
 */

import { useState, useEffect, useCallback, useRef } from 'react';
import type { ApexDashboard, TrafficDataPoint, AttackTypeData } from '../types/apex';
...
export interface UseApexDashboardOptions {
...
export interface UseApexDashboardResult {
  data: ApexDashboard | null;
  isLoading: boolean;
  error: Error | null;
...
function generateDemoData(): ApexDashboard {
  const now = new Date();
...
export function useApexDashboard(options: UseApexDashboardOptions = {}): UseApexDashboardResult {
  const {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${apiBaseUrl}/apex/dashboard`, {
      ...
      const dashboardData: ApexDashboard = {
      ...
      console.warn('Failed to fetch apex dashboard, using demo data:', err);
...
export default useApexDashboard;
