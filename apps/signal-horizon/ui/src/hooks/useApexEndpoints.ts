...
export interface UseApexEndpointsOptions {
...
export interface UseApexEndpointsResult {
  endpoints: Endpoint[];
  services: Service[];
  isLoading: boolean;
  error: Error | null;
...
export function useApexEndpoints(options: UseApexEndpointsOptions = {}): UseApexEndpointsResult {
  const {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const url = `${apiBaseUrl}/apex/endpoints${params.toString() ? `?${params}` : ''}`;
      ...
      console.warn('Failed to fetch apex endpoints, using demo data:', err);
  ...
  const fetchEndpointById = useCallback(async (id: string): Promise<Endpoint | null> => {
    try {
      const response = await fetch(`${apiBaseUrl}/apex/endpoints/${id}`, {
        headers: { 'Accept': 'application/json' },
      });
...
export default useApexEndpoints;
