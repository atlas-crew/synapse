...
import type { Rule, RuleDeployment, RuleTemplate, RuleCategory, RuleSeverity, RuleAction } from '../types/apex';
...
export interface UseApexRulesOptions {
...
export interface UseApexRulesResult {
  rules: Rule[];
...
export function useApexRules(options: UseApexRulesOptions = {}): UseApexRulesResult {
  const {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${apiBaseUrl}/apex/rules`, {
      ...
      console.warn('Failed to fetch apex rules, using demo data:', err);
  ...
  const fetchRuleById = useCallback(async (id: string): Promise<Rule | null> => {
    try {
      const response = await fetch(`${apiBaseUrl}/apex/rules/${id}`, {
        headers: { 'Accept': 'application/json' },
      });
