...
interface BeamErrorBoundaryProps {
  children: ReactNode;
  title?: string;
  description?: string;
...
interface BeamErrorBoundaryState {
  hasError: boolean;
  error: Error | null;
}

export class BeamErrorBoundary extends React.Component<
  BeamErrorBoundaryProps,
  BeamErrorBoundaryState
> {
  constructor(props: BeamErrorBoundaryProps) {
    super(props);
    this.state = {
      hasError: false,
      error: null,
  ...
  static getDerivedStateFromError(error: Error): BeamErrorBoundaryState {
    return {
      hasError: true,
      error,
    };
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    ...
    console.error(
      `[BeamErrorBoundary - ${level}] Error caught:`,
      error,
      errorInfo
    );
  }
