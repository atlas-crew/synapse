    ...
    <div className="p-6 space-y-6 min-h-screen bg-surface-base">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <MetricCard
          ...
          description="Cumulative response bodies scanned for sensitive data patterns"
        />
        <MetricCard
          ...
          className={
            stats && stats.totalMatches > 0
              ? 'border-ac-red/40 shadow-[0_0_10px_rgba(239,68,68,0.1)]'
              : ''
          }
        />
        <MetricCard
          label="Active Patterns"
          value={stats?.patternCount?.toString() || '0'}
          description="Number of DLP detection patterns currently enabled"
        />
        <MetricCard
          label="Avg Scan Time"
          value="42μs"
          description="Average time to scan a single response body for sensitive data"
          className="text-ac-blue"
        />
      </div>
              ...
              {violationsLoading ? (
                ...
                violations.map((v, idx) => (
                  <tr key={idx} className="hover:bg-ac-blue/5 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div
                        className="text-xs text-ink-muted truncate max-w-[180px] font-mono"
                        title={v.path}
                      >
                        {v.path}
                      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-12">
        <div className="card p-6 bg-surface-card border border-border-subtle">
          ...
          <div className="space-y-6">
            ...
            />
            <CoverageItem
              label="Corporate IP (Custom Dictionaries)"
              status="Monitoring"
              progress={100}
              color="bg-ac-blue"
            />
          ...
          <div className="flex-1 flex items-center justify-center text-ink-muted border border-dashed border-border-subtle bg-surface-subtle/30">
            <div className="text-center">
              ...
              <p className="text-xs font-mono uppercase tracking-widest opacity-40">
                Classification Analytics Coming Soon
              </p>
            </div>
          </div>
        </div>
