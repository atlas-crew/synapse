/**
 * Apex Analytics Routes
 * Proxies and aggregates data from risk-server for the Apex analytics dashboard
 */
...
export function createApexRoutes(logger: Logger): Router {
  const router = Router();
  const log = logger.child({ module: 'apex-routes' });

  /**
   * GET /api/v1/apex/analytics
  ...
  router.get('/analytics', async (_req: Request, res: Response, _next: NextFunction) => {
    log.debug('Fetching apex analytics data');
    ...
    const response: ApexAnalyticsResponse = {
  ...
  log.info('Apex analytics routes initialized');
  return router;
}
